...
Для того, чтобы получить копию репозитория на свой компьютер, сначала скопируйте URL вашего проекта (Clone with SSH не потребует ввода имени пользователя и пароля GitLab, в отличие от Clone with HTTPS):
...
Добавте преподавательский репозиторий в список удаленных репозиториев (делается один раз):
```bash
